...
${interpolate(gettext("The <code>gerrit-reviewboard</code> plugin is required for Gerrit integration. See the <a href=\"%s\" target=\"_blank\">instructions</a> for installing the plugin on your server."), [MANUAL_URL + 'admin/configuration/repositories/gerrit/'])}
  ...
  $(document).ready(function () {
    ...
    $bugTrackerPlan.change(() => {
      ...
      $bugTrackerUsernameRow.toggle(planInfo.bug_tracker_requires_username);
    });
    $hostingType.change(() => {
    ...
    $([$hostingType[0], $hostingAccount[0]]).change(() => {
      ...
      if (hostingType === 'custom') {
        ...
        if (hostingInfo.fake !== true) {
           ...
           * needed.
           */
          $authForm.find('[data-required-for-2fa]').closest('.form-row').toggle(hostingInfo.needs_two_factor_auth_code);
    ...
    $bugTrackerType.change(() => {
      ...
      if (bugTrackerType === 'custom') {
        ...
        $bugTrackerHostingURLRow.toggle(HOSTING_SERVICES[bugTrackerType].self_hosted);
      }
    }).triggerHandler('change');
