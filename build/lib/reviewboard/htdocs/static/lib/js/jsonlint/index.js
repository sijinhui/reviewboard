(function (factory) {
	...
	var jsonlintExports = {};
	var jsonlint$1 = {
	  get exports(){ return jsonlintExports; },
	  set exports(v){ jsonlintExports = v; },
	};
	  ...
	  (function (global, factory) {
	    ...
	    const Uni = {
	      ...
	      isWhiteSpace: function isWhiteSpace(x) {
	        // section 7.2, table 2
	        return x === '\u0020' || x === '\u00A0' || x === '\uFEFF' ||
	        // <-- this is not a Unicode WS, only a JS one
	        x >= '\u0009' && x <= '\u000D' ||
	        // 9 A B C D
	      ...
	      isIdentifierPart: function isIdentifierPart(x) {
	        return x === '$' || x === '_' || x >= 'A' && x <= 'Z' || x >= 'a' && x <= 'z' || x >= '0' && x <= '9' ||
	        // <-- addition to Start
	    ...
	    function parseInternal(input, options) {
	      ...
	      function parseGeneric() {
	        if (position < inputLength) {
	          ...
	          if (char === '"' || char === '\'' && allowSingleQuotedStrings) {
	            ...
	            endToken && endToken('symbol', '[');
	            return parseArray();
	          } else if (char === '-' || char === '.' || isDecDigit(char) || json5 && (char === '+' || char === 'I' || char === 'N')) {
	      ...
	      function parseKey() {
	        ...
	        if (position < inputLength) {
	          ...
	          if (char === '"' || char === '\'' && allowSingleQuotedStrings) {
	            ...
	            endToken && endToken('literal', number);
	            return number;
	          } else if (json5 && Uni.isIdentifierStart(char) || char === '\\' && input[position] === 'u') {
	      ...
	      function parseObject() {
	        ...
	        while (position < inputLength) {
	          ...
	          if (char === '}' && key === undefined) {
	            ...
	            if (key in emptyObject || emptyObject[key] != null) ; else {
	              if (reviver) {
	      ...
	      function parseNumber() {
	        ...
	        if (char === '0') {
	          ...
	          if (json5 && (isOctal || isHex)) {
	            while (position < inputLength && (isHex ? isHexDigit : isOctDigit)(input[position])) {
	              ++position;
	            }
	            let sign = 1;
	            if (input[start] === '-') {
	      ...
	      function parseIdentifier() {
	        ...
	        while (position < inputLength) {
	          let char = input[position++];
	          if (char === '\\' && input[position] === 'u' && isHexDigit(input[position + 1]) && isHexDigit(input[position + 2]) && isHexDigit(input[position + 3]) && isHexDigit(input[position + 4])) {
	    ...
	    function getLineAndColumn(input, offset) {
	      const lines = input.substr(0, offset).split(/\r?\n/);
	    ...
	    function getReason(error) {
	      let message = error.message.replace('JSON.parse: ', '') // SpiderMonkey
	    ...
	    function needsCustomParser(options) {
	      return options.ignoreBOM || options.ignoreComments || options.ignoreTrailingCommas || options.allowSingleQuotedStrings || options.allowDuplicateObjectKeys === false || options.mode === 'cjson' || options.mode === 'json5' || isSafari || oldNode;
	    }
	...
	})(jsonlint$1, jsonlintExports);
	const jsonlint = /*@__PURE__*/getDefaultExportFromCjs(jsonlintExports);
